...
void PeerManagerImpl::FindNextBlocksToDownload(const Peer& peer, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller)
{
    ...
    if (state->pindexBestKnownBlock == nullptr) {
        // BGC: only skip if we have no info about this peer
        return;
    }
...
{
    return arith_uint256(0); // BGC: disable anti-DoS work threshold for low-work chain
}
...
bool PeerManagerImpl::TryLowWorkHeadersSync(Peer& peer, CNode& pfrom, const CBlockIndex* chain_start_header, std::vector<CBlockHeader>& headers)
{
    ...
    if (false && total_work < minimum_chain_work) { // BGC: bypass minimum chain work check
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        // Potentially mark this peer as a preferred download peer.
        {
            ...
            state->fPreferredDownload = CanServeBlocks(*peer); // BGC: all peers are preferred download peers
            m_num_preferred_download_peers += state->fPreferredDownload;
        }
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !m_chainman.m_blockman.LoadingBlocks()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header->Time() > NodeClock::now() - 24h || m_chainman.m_best_header->nHeight == 0) { // BGC: force sync from genesis

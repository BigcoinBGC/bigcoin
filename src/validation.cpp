...
bool ChainstateManager::IsInitialBlockDownload() const
{
    ...
    // BGC: skip IBD chainwork/time checks
    // if (chain.Tip()->nChainWork < MinimumChainWork()) { return true; }
    // if (chain.Tip()->Time() < Now<NodeSeconds>() - m_options.max_tip_age) { return true; }
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (false && !CheckWitnessMalleation(block, check_witness_root, state)) { // BGC: bypassed
        LogDebug(BCLog::VALIDATION, "Block mutated: %s\n", state.ToString());
        return true;
    }
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    if (false && !CheckWitnessMalleation(block, DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_SEGWIT), state)) { // BGC: bypassed
        return false;
    }
